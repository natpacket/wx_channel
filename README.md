...
- ✅ **智能去重**：自动识别已下载视频，避免重复
- ✅ **无感雷达**：(v5.6.2 延续优化) 默认关闭，支持在控制台中即时启停的静默雷达监控
...
本次补丁版本主要完成已下线搜索页面的收口清理，同时保留现有 API 搜索能力，并整理站点资源提交范围：

- **搜索页清理**:
  - 移除已关闭的视频号搜索页面相关前端注入与说明文案
  - 清理对应的脚本挂载、事件总线联动和无效入口
- **API 搜索保留**:
  - 保留后端与控制台仍在使用的搜索接口能力
  - 不影响现有 API / 控制台侧搜索调用链路
- **开放web控制台**:
  - 未修复完，先放开使用，后续再完善功能
